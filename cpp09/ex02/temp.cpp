...
/*   Updated: 2025/04/05 20:12:16 by pipolint         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	PMergeMe::recurseVector(int pairSize)
{
	...
	const unsigned int	nPairs = pairElements.size();
	...
	while (appendageCount > 0)
	{
		...
		std::cout << "current jacobsthal: " << calculateJacobsthal(nthJacobsthal) << "\n";

		if (pairElements.at(currentJacobsthal - 1).second != -1)
			mainChainBound = mainChain.begin() + (pairSize - 1) * (calculateJacobsthal(nthJacobsthal) - 1) + 1;
		else
		{
			std::cout << "\033[31mNo corresponding a element for bound\033[0m\n";
		...
		std::cout << "\033[34m";
		std::for_each(mainChain.begin(), mainChainBound, printElems);
		std::cout << "\n\033[0m";
		std::cout << "Bound element: " << *mainChainBound << "\n";
		std::cout << "\033[33mUpper bound for element " << this->vector.at(pairElements.at(currentJacobsthal - 1).first) << ": " << *upperBound << "\033[0m\n";
		...
		std::cout << "\033[32minserting elements: ";
